...
namespace Weber.Scripts.Legend.Skill
{
    public class SpellShootProp : BattleProp
    {
        ...
        private void Shot(SpellMuzzle muzzle)
        {
            ...
            Transform target;
            if (targetCharater == null) target = null;
            else target = targetCharater.transform;

            Vector3 forword = Vector3.forward;
            if (target != null && _tracerTarget != TracerTarget.None)
            {
            ...
            spellBullet.SetFromSource(this, _mode, _pierce, _tracerDeviation, _speed, _lifeTime, _maxDistance, _hitLayer, direction, target,
        ...
        private CharacterUnit FindTarget()
        {
            switch (_tracerTarget)
            {
                ...
                case TracerTarget.LowestHealth:
                    return EnemyFactory.Instance.FindLowestHealthEnemy(CharacterUnit.transform.position, _maxDistance);
                default:
                    return null;
            }
        }
